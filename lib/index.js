...
(function () {
  ...
  var pakmanager = module.exports
    ...
    , future = Future()
    , enderJs = path.join(__dirname, '..', 'vendor', 'ender-js', 'ender.js')
  ...
  pakmanager.init = function (params, args, fn) {
    pakman.getPackageInfo(packageRoot, function (err, pkg) {
      if (err) {
  ...
  pakmanager.build = function () {
    pakman.compile(packageRoot, templateModule, function (err, compiled) {
      fs.writeFile(path.join(packageRoot, 'pakmanaged.js'), ''
        //+ '(function () {\n'
